...
PROJECT_NAME=deporeibar-baietz-312
STACK_NAME=312-deporeibar-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://deporeibar-baietz-312.localhost"
...
acceptance-frontend-image-build: ## Build Acceptance frontend server image
	...
	@docker build frontend -t codesyntax/deporeibar-baietz-312-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build: ## Build Acceptance backend server image
	@echo "Build acceptance backend"
	@docker build backend -t codesyntax/deporeibar-baietz-312-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start: ## Start Acceptance frontend container
	...
	@docker run --rm -p 3000:3000 --name deporeibar-baietz-312-frontend-acceptance --link deporeibar-baietz-312-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d codesyntax/deporeibar-baietz-312-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start: ## Start Acceptance backend container
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name deporeibar-baietz-312-backend-acceptance -d codesyntax/deporeibar-baietz-312-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop deporeibar-baietz-312-frontend-acceptance
	@docker stop deporeibar-baietz-312-backend-acceptance
