...
class ContentsUploadModal extends Component {
  ...
  onDrop = async (acceptedFiles, fileRejections, event) => {
    if (fileRejections.length > 0) {
      const fitxategiak = fileRejections.map((item) => {
        const mezuak = item.errors.map((error) => error.message);
        const erroreMezuak = mezuak.join(', ');

        return `${item.file.path}: ${erroreMezuak}`;
      });
      const fitxategienMezua = fitxategiak.join(', ');
      toast.warning(
        <Toast
          autoClose={3000}
          title={`Errorea fitxategiak aukeratzean`}
          content={`Fitxategi hauek ezin dituzu kargatu: ${fitxategienMezua}`}
        />,
      );
    }
    ...
    for (let i = 0; i < acceptedFiles.length; i++) {
      if (
        validateFileUploadSize(acceptedFiles[i], this.props.intl.formatMessage)
      ) {
        await readAsDataURL(acceptedFiles[i]).then((data) => {
          const fields = data.match(/^data:(.*);(.*),(.*)$/);
          acceptedFiles[i].preview = fields[0];
        });
        validFiles.push(acceptedFiles[i]);
      }
    }
    this.setState({
    ...
    Promise.all(map(this.state.files, (file) => readAsDataURL(file))).then(
      (files) => {
        this.props.createContent(
          expandToBackendURL(this.props.pathname),
